...
describe("taking lines out of a region", () => {
  ...
  it("writes the sides in the order they were taken", () => {
    expect(
      render(two, { 0: [...take("theirs", 0, 1), ...take("ours", 0, 1)] }),
    ).toBe("top\nyours one\nyours two\nmine one\nmine two\nend\n");
  });
...
describe("picking sides region by region", () => {
  ...
  it("ends a hand-typed result with a newline, because a text file has one", async () => {
    ...
    merge.active = "f.txt";
    const sent: string[] = [];
    vi.spyOn(commands, "resolveConflict").mockImplementation(
      async (_p, _f, choice) => {
...
describe("arriving at a stopped operation", () => {
  ...
  function wire(
    ...
    vi.spyOn(commands, "repoOperation").mockResolvedValue({
      ...
      labels: {
        ours: "dummyx",
        theirs: "056d9dc (lets see what happens)",
        swapped: false,
      },
      progress: null,
      headName: null,
    ...
    vi.spyOn(commands, "conflictBlocks").mockImplementation(
      async (_p, file) =>
        ({
          blocks: [
            {
